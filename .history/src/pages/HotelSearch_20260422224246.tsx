...
const getHotelImage = (hotel: any, index: number) => {
  const query = `${hotel.city || "hotel"} luxury hotel`;

  return `https://api.unsplash.com/photos/random?query=${encodeURIComponent(query)}&client_id=${import.meta.env.VITE_UNSPLASH_KEY}&sig=${index}`;
};
                  ...
                  <Card className="h-full flex flex-col border border-border/50 bg-card overflow-hidden rounded-[20px] shadow-lg hover:shadow-xl transition-all duration-300 hover:scale-[1.02]">
                    <div className="relative h-[200px] w-full overflow-hidden shrink-0">
                      <SafeImage
                        src={
                          hotel.imageUrl ||
                          `https://images.unsplash.com/photo-1501117716987-c8e1ecb2102d?auto=format&fit=crop&w=600&q=80&sig=${hotel._id || i}`
                        }
                        alt={hotel.name}
                      />
