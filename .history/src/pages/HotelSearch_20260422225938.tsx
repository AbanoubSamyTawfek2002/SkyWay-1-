...
const getHotelImage = async (hotel: any, index: number) => {
  const query = `${hotel.city || "luxury"} hotel`;
  ...
  const fetchHotels = useCallback(
    async (isLoadMore = false) => {
      ...
      try {
        ...
        // 🔥 أهم جزء: نجيب صورة لكل فندق
        const hotelsWithImages = await Promise.all(
          result.data.map(async (hotel: any, i: number) => {
            const image = await getHotelImage(hotel, i);
                ...
                >
                  <Card className="h-full flex flex-col border border-border/50 bg-card overflow-hidden rounded-[20px] shadow-lg hover:shadow-xl transition-all duration-300 hover:scale-[1.02]">
                    <div className="relative h-[200px] w-full overflow-hidden shrink-0">
                      <SafeImage src={hotel.imageUrl} alt={hotel.name} />
