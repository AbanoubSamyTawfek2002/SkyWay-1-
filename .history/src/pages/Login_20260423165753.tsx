...
// 🛑 أهم سطر: استيراد رابط الـ API الموحد
...
export default function Login() {
  ...
  useEffect(() => {
    const handleOauthMessage = (event: MessageEvent) => {
      // السماح بالاستقبال من Vercel أو Localhost
      if (
  ...
  const handleGoogleLogin = async () => {
    try {
      // 🔄 تعديل الرابط ليستخدم العنوان الكامل
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // 🔄 تعديل الرابط ليتصل بـ Vercel مباشرة
    ...
    <div className="container mx-auto flex items-center justify-center min-h-[calc(100vh-64px)] px-4 py-8">
      <Card className="w-full max-w-md shadow-2xl border-t-4 border-t-primary">
        <CardHeader className="space-y-1 text-center">
          ...
          <CardDescription>
            Enter your credentials to access your SkyWay account
          </CardDescription>
        </CardHeader>
        <CardContent>
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            {/* Mock CAPTCHA */}
            <div className="p-4 bg-muted/50 rounded-lg flex items-center gap-4 border border-border">
              <Checkbox
                id="captcha"
            ...
            <Button
              type="button"
              variant="outline"
              className="w-full h-12 font-bold uppercase tracking-widest gap-3 hover:bg-muted"
