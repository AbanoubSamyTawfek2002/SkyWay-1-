...
export default function HotelDetails() {
  ...
  useEffect(() => {
    // إيقاف التنفيذ لو مفيش ID عشان نمنع الأخطاء غير الضرورية
    if (!id) return;

    const fetchData = async () => {
      try {
        setLoading(true);
        // 🔄 التعديل هنا: استخدام API_URL لطلب تفاصيل الفندق
  ...
  if (loading)
    return (
      <div className="container mx-auto p-20 text-center flex flex-col items-center justify-center min-h-[60vh] gap-6">
  ...
  if (error || !hotel) {
    return (
      <div className="container mx-auto p-20 text-center min-h-[60vh] flex flex-col items-center justify-center">
