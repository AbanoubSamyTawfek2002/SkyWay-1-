...
export default function HotelSearch() {
  ...
  const fetchHotels = async (isLoadMore = false) => {
    ...
    try {
      // 1. حساب تواريخ ديناميكية عشان الـ API ميضربش في المستقبل
      const getFutureDate = (daysToAdd) => {
        const date = new Date();
        date.setDate(date.getDate() + daysToAdd);
        return date.toISOString().split("T")[0]; // بيطلع التاريخ بصيغة YYYY-MM-DD
      };

      const checkIn = getFutureDate(7); // حجز كمان أسبوع
      const checkOut = getFutureDate(10); // مغادرة كمان 10 أيام

      // 2. ضفنا arrival_date و departure_date للرابط
      const url = `https://booking-com15.p.rapidapi.com/api/v1/hotels/searchHotels?dest_id=-2092174&search_type=CITY&arrival_date=${checkIn}&departure_date=${checkOut}&adults=1&room_qty=1&page_number=1&units=metric&temperature_unit=c&languagecode=en-us&currency_code=AED`;
      ...
      const res = await fetch(url, {
        ...
        headers: {
          "x-rapidapi-key":
            ...
            "acf0d045edmsh3e2c6767325c2b7p126f7fjsna92c42c0181f", // حطيت الكي مؤقتاً هنا كاحتياطي
      ...
      console.log("API Response Fixed:", result); // عشان تتأكد إن الـ status بقت true
