...
import { Plane, Clock, ArrowRight, Star, ChevronDown, Calendar, Users, MapPin, Search } from 'lucide-react';
...
export default function FlightSearch() {
  ...
  const [activeRoute, setActiveRoute] = useState(searchParams.get('from') && searchParams.get('to') ? `${searchParams.get('from')} → ${searchParams.get('to')}` : 'All Routes');
    ...
    <div className="container mx-auto px-4 sm:px-6 md:px-10 py-12 sm:py-20">
      <div className="flex flex-col xl:flex-row justify-between items-start xl:items-end mb-16 gap-10">
        <div className="max-w-2xl">
          <span className="inline-block px-4 py-1.5 rounded-full bg-primary/10 text-primary font-black uppercase tracking-widest text-[10px] mb-4 italic">SkyWay Elite Board</span>
          <h1 className="text-6xl md:text-8xl font-black tracking-tighter uppercase italic leading-none mb-6">Global Wings</h1>
          <p className="text-muted-foreground text-lg sm:text-xl italic">Book premium routes across the globe with our real-time intelligent flight board.</p>
        </div>
                    ...
                    <div className="flex flex-col lg:flex-row items-stretch">
                      ...
                      <div className="lg:w-1/4 p-10 bg-muted/20 flex flex-col items-center justify-center border-b lg:border-b-0 lg:border-r border-border/50">
                        ...
                        <div className="text-center">
                          <h3 className="font-black text-2xl tracking-tighter uppercase italic leading-none mb-2">{flight.airline}</h3>
                          <Badge variant="outline" className="rounded-full px-4 py-1 font-bold text-[10px] uppercase tracking-widest border-primary/20 text-primary">{flight.flightNumber}</Badge>
                        </div>
                      </div>
                      ...
                      <div className="lg:w-2/4 p-8 sm:p-12 flex flex-col md:flex-row items-center justify-between gap-12 flex-1">
                        <div className="text-center md:text-left min-w-[120px]">
                          <p className="text-[10px] font-black uppercase tracking-[0.3em] text-muted-foreground mb-3">{flight.departureAirport}</p>
                          <h4 className="text-5xl font-black tracking-tighter italic leading-none mb-2">{new Date(flight.departureTime).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}</h4>
                          <p className="font-black text-primary uppercase text-sm italic">{flight.departureCity}</p>
                        </div>
                        ...
                        <div className="flex-1 w-full flex flex-col items-center gap-4">
                          ...
                          <div className="flex items-center gap-4">
                            ...
                            <span className="text-[9px] font-black uppercase tracking-widest text-muted-foreground/60 italic">{flight.class} CLASS</span>
                          </div>
                        </div>

                        <div className="text-center md:text-right min-w-[120px]">
                          <p className="text-[10px] font-black uppercase tracking-[0.3em] text-muted-foreground mb-3">{flight.arrivalAirport}</p>
                          <h4 className="text-5xl font-black tracking-tighter italic leading-none mb-2">{new Date(flight.arrivalTime).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}</h4>
                          <p className="font-black text-primary uppercase text-sm italic">{flight.arrivalCity}</p>
                        </div>
                      </div>
                      ...
                      <div className="lg:w-1/4 p-10 bg-primary/5 flex flex-row lg:flex-col justify-between lg:justify-center items-center lg:items-end lg:border-l border-border/50">
                        <div className="text-left lg:text-right mb-6">
                           <div className="flex items-center lg:justify-end gap-2 text-primary font-black mb-3">
                            ...
                            <span className="text-xl italic">{flight.rating?.toFixed(1) || '4.5'}</span>
                            <span className="text-[10px] text-muted-foreground font-bold italic ml-2">({flight.reviewCount || 0} REVIEWS)</span>
                          </div>
                          <div className="flex items-baseline gap-1 lg:justify-end">
                            <span className="text-5xl font-black tracking-tighter italic">{formatPrice(flight.price)}</span>
                          </div>
                        </div>
                        <div className="flex flex-col gap-3 w-full max-w-[200px]">
                          <div className="flex gap-2">
                            ...
                            <WishlistButton itemType="flight" itemId={flight._id} className="h-14 w-14 rounded-2xl border-2 shadow-sm" />
                          </div>
                          <Button 
                            variant="outline"
                            className="w-full h-12 rounded-2xl font-black uppercase tracking-widest text-[9px] border-2 italic hover:bg-primary/5"
                            onClick={() => setExpandedReview(expandedReview === flight._id ? null : flight._id)}
                          >
                            {expandedReview === flight._id ? 'Close Records' : 'Rating Records'}
                          </Button>
                        </div>
                      </div>
                    </div>
                          ...
                          <div className="p-12">
                            <ReviewSection 
                              ...
                              onReviewAdded={(avg, count) => {
                                setFlights(prev => prev.map(f => f._id === flight._id ? { ...f, rating: avg, reviewCount: count } : f));
                              }}
                            />
                          </div>
                        </motion.div>
            ...
            <div className="mt-20 flex justify-center">
              ...
              >
                {loadingMore ? (
                  ...
                  <>
                    More Flights <ChevronDown className="ml-2 group-hover:translate-y-1 transition-transform" />
                  </>
                )}
              </Button>
            </div>
          ...
          {flights.length === 0 && (
            <div className="py-40 text-center flex flex-col items-center gap-8 bg-muted/20 rounded-[60px] border-2 border-dashed">
              ...
              <div className="max-w-md">
                <h2 className="text-3xl font-black uppercase italic tracking-tighter mb-4">Routes Busy</h2>
                <p className="text-muted-foreground italic mb-8">No premium wings available for your current trajectory. Select a different route or filter.</p>
                <Button variant="outline" className="rounded-full px-8 font-bold uppercase tracking-widest text-[10px]" onClick={() => handleRouteSelect(routeCategories[0].name)}>Clear Trajectory</Button>
              </div>
            </div>
          )}
        </div>
