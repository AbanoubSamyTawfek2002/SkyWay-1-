...
import { MapPin, Star, Wifi, Droplets, Dumbbell, Coffee, Utensils, Sparkles, User, Hotel as HotelIcon, ChevronDown } from 'lucide-react';
...
export default function HotelSearch() {
  ...
  const fetchHotels = useCallback(async (isLoadMore = false) => {
  ...
  const handleCategorySelect = (val: string) => {
    ...
    if (val === 'All') {
      ...
      const countries = ['Egypt', 'UAE', 'France', 'UK', 'USA', 'Japan', 'Italy', 'Singapore'];
    ...
    <div className="container mx-auto px-4 sm:px-6 md:px-10 py-12 sm:py-20">
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-end mb-16 gap-10">
        <div className="max-w-2xl">
          <span className="inline-block px-4 py-1.5 rounded-full bg-primary/10 text-primary font-black uppercase tracking-widest text-[10px] mb-4 italic">Exclusive Selection</span>
          <h1 className="text-6xl md:text-8xl font-black tracking-tighter uppercase italic leading-none mb-6">Luxe Stays</h1>
          <p className="text-muted-foreground text-lg sm:text-xl italic">Curating the world's most breathtaking accommodations for your prestige journey.</p>
        </div>
      ...
      {loading ? (
        <div className="grid grid-cols-[repeat(auto-fit,minmax(280px,1fr))] gap-[20px]">
          {[...Array(8)].map((_, i) => <SkeletonHotelCard key={i} />)}
        </div>
      ) : (
        <>
          <div className="grid grid-cols-[repeat(auto-fit,minmax(280px,1fr))] gap-[20px]">
                ...
                >
                  <Card className="h-full flex flex-col border border-border/50 bg-card overflow-hidden rounded-[20px] shadow-lg hover:shadow-xl transition-all duration-300 hover:scale-[1.02]">
                    <div className="relative h-[200px] w-full overflow-hidden shrink-0">
                      <SafeImage 
                        src={hotel.images?.[0] || `https://source.unsplash.com/featured/?hotel,${hotel.city.toLowerCase()}`} 
                        alt={hotel.name} 
                      ...
                      <div className="absolute top-3 left-3 z-10">
                        <Badge className="bg-primary/95 text-white font-black px-3 py-1 rounded-lg shadow-lg text-[11px] uppercase tracking-tighter italic">
                          ...
                          <span className="text-[8px] ml-1 opacity-70 not-italic font-bold">/ NIGHT</span>
                        </Badge>
                      </div>
                      <div className="absolute top-3 right-3 z-10">
                        <WishlistButton itemType="hotel" itemId={hotel._id} className="h-9 w-9 bg-black/20 backdrop-blur-md border-none text-white hover:bg-black/40" />
                      </div>
                    ...
                    <CardContent className="p-5 flex flex-col flex-1">
                      <div className="flex justify-between items-start mb-3 gap-2">
                        <div className="min-w-0">
                          <h3 className="font-black text-xl tracking-tight uppercase italic leading-none mb-2 truncate group-hover:text-primary transition-colors">{hotel.name}</h3>
                          <div className="flex items-center gap-1.5 text-muted-foreground text-[11px] font-bold uppercase tracking-widest">
                            <MapPin size={12} className="text-primary shrink-0" />
                            <span className="truncate italic">{hotel.city}, {hotel.country}</span>
                          </div>
                        </div>
                        <div className="flex items-center gap-1 bg-primary/10 text-primary px-2 py-1 rounded-lg shrink-0">
                          <Star size={14} fill="currentColor" />
                          <span className="font-black text-sm italic">{hotel.rating?.toFixed(1)}</span>
                        </div>
                      </div>
            ...
            <div className="mt-20 flex justify-center">
              ...
              >
                {loadingMore ? (
                  ...
                  <>
                    Explore More <ChevronDown className="ml-2 group-hover:translate-y-1 transition-transform" />
                  </>
                )}
              </Button>
            </div>
          ...
          {hotels.length === 0 && (
            <div className="py-40 text-center flex flex-col items-center gap-8 bg-muted/20 rounded-[60px] border-2 border-dashed">
              ...
              <div className="max-w-md">
                <h2 className="text-3xl font-black uppercase italic tracking-tighter mb-4">No Stays Found</h2>
                <p className="text-muted-foreground italic mb-8">We couldn't find any premium properties matching your current filters. Try refining your selection.</p>
                <Button variant="outline" className="rounded-full px-8 font-bold uppercase tracking-widest text-[10px]" onClick={() => handleCategorySelect('')}>Clear All Filters</Button>
              </div>
            </div>
          )}
        </>
