...
async function startServer() {
  ...
  // MongoDB Connection
  const MONGODB_URI = process.env.MONGODB_URI || "mongodb+srv://pepoabanob7_db_user:uQGUg6ZRmIbrejd4@cluster0.q6ypfrw.mongodb.net/?appName=Cluster0";
  ...
  try {
    ...
    console.error("CRITICAL: MongoDB connection failed. Please ensure your IP is whitelisted in MongoDB Atlas (Allow 0.0.0.0/0 for testing).", err);
  }
    ...
    try {
      ...
      if (reviewCount === 0) {
        ...
        for (const hotel of hotels) {
          ...
          // Update average
          const avgRating = reviewsToInsert.reduce((acc, curr) => acc + curr.rating, 0) / reviewsToInsert.length;
          await Hotel.findByIdAndUpdate(hotel._id, { rating: avgRating, reviewCount: reviewsToInsert.length });
        }
        ...
        for (const flight of flights) {
          ...
          // Update average
          const avgRating = reviewsToInsert.reduce((acc, curr) => acc + curr.rating, 0) / reviewsToInsert.length;
          await Flight.findByIdAndUpdate(flight._id, { rating: avgRating, reviewCount: reviewsToInsert.length });
        }
